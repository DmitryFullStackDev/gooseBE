...
import { RoundDetails, WinnerInfo } from '../types/round.types'
...
export class RoundService {
  ...
  constructor(
    ...
    @InjectModel(UserRoundStats)
    private userRoundStatsModel: typeof UserRoundStats,
  ...
  async setWinner(roundId: number, userId: number) {
    await this.sequelize.transaction(async t => {
      ...
      if (winnerStats) {
        this.logger.log(
          `Setting winner for round ${roundId}: User ${winnerStats.dataValues.userId} with ${winnerStats.dataValues.points} points`,
          winnerStats,
        )

        await this.sequelize.query('UPDATE "Rounds" SET "winnerId" = ? WHERE id = ?', {
