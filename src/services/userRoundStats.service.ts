...
export class UserRoundStatsService {
  ...
  async incrementTapAndPoints(userId: number, roundId: number): Promise<UserRoundStats> {
    return this.sequelize.transaction(async t => {
      ...
      const totalPoints = allStats.length
        ...
        : 0
      await this.roundModel.update({ totalPoints }, { where: { id: roundId }, transaction: t })
